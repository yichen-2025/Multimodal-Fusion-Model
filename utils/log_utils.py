...
def save_log(log_type, log_data):
    ...
    Args:
        log_type (str): 日志类型，可选值：
            ...
            - 'training'      : 模型训练日志
            - 'testing'       : 模型测试日志
            - 'openset_split' : 开集数据划分日志
            - 'ood_training'  : OOD检测头训练日志
            - 'ood_routing'   : OOD路由评估日志
            - 'fewshot_split' : 少样本数据划分日志
            - 'experiment'    : 实验运行日志
    ...
    index_columns = {
        ...
        'openset_split': ['log_id', 'timestamp', 'dataset_id', 'source_split_id', 'output_split_id', 'unknown_ratio', 'train_known', 'val_known', 'test_known', 'test_unknown', 'test_benign'],
        'ood_training': ['log_id', 'timestamp', 'model_id', 'dataset_id', 'split_id', 'feature_dim', 'num_classes', 'distance_type', 'threshold', 'train_known', 'val_total', 'duration_seconds'],
        'ood_routing': ['log_id', 'timestamp', 'dataset_id', 'split_id', 'backbone_model_id', 'ood_id', 'llm_model_id', 'total_test', 'macro_f1', 'unknown_f1', 'duration_seconds'],
        'fewshot_split': ['log_id', 'timestamp', 'dataset_id', 'source_split_id', 'output_split_id', 'k_per_class', 'num_known_classes', 'train_total', 'val_total', 'test_total'],
        'experiment': ['log_id', 'timestamp', 'dataset_id', 'split_id', 'backbone_model_id', 'ood_id', 'llm_model_id', 'fewshot_k', 'stage', 'status', 'metrics_json', 'duration_seconds']
    }
